<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PHP Week 3</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
</head>
<body>
    <div class="container">
        <h1 class="display-3">Welcome to Week 3 of PHP!</h1>
        <p class="lead">Here's what we'll talk about today:</p>
        <ol>
            <li><a href="../html/takeTheQuiz.html">Take the quiz</a></li>
            <li>Review last week and online material</li>
            <li><a href="integratingPHPAndMySQL.php">Integrating PHP & MySQL</a></li>
            <li><a href="creatingDatabases.php">Creating Databases</a></li>
            <li><a href="creatingTablesWithDifferentFields.php">Creating Tables With Different Fields</a></li>
            <li><a href="addingDataToTables.php">Adding Data to Tables</a></li>
            <li><a href="preparedStatements.php">Prepared Statements</a></li>
        </ol>
        <p class="lead">And here are the activities we'll do:</p>
        <ol>
            <li><a href="creatingDatabasesWithTables.php">PHP & MySQL</a></li>
        </ol>
    </div>
    <script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.0/dist/umd/popper.min.js" integrity="sha384-Q6E9RHvbIyZFJoft+2mJbHaEWldlvI9IOYy5n3zV9zzTtmI3UksdQRVvoxMfooAo" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.min.js" integrity="sha384-wfSDF2E50Y2D1uUdj0O3uMBJnjuUD4Ih7YwaYd1iqfktj0Uod8GCExl3Og8ifwB6" crossorigin="anonymous"></script>
</body>
</html>